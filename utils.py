...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key, model_name, patch_size,
                            remove_backbone_prefix=True, filter_if_error=True):
    if os.path.isfile(pretrained_weights):
        ...
        if remove_backbone_prefix:
            state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items()}
        if not filter_if_error:
            ...
            try:
                ...
                state_dict, non_matching = filter_state_dict(state_dict, model)
                msg = model.load_state_dict(state_dict, strict=False)
                print('Pretrained weights found at {} and filtered of non-matching size weights {} '
                      'and loaded with msg: {}'.format(pretrained_weights, non_matching, msg))
...
def filter_state_dict(state, model):
    model_state = model.state_dict()
    non_matching_sizes = set([k for k, v in state.items() if v.size() != model_state[k].size()])
    new_state = {k: v for k, v in state.items() if k not in non_matching_sizes}
    return new_state, non_matching_sizes
...
def cancel_gradients_backbone(epoch, model, freeze_till):
    if epoch >= freeze_till:
        return
    for n, p in model.named_parameters():
        if "backbone" in n:
            p.grad = None
...
def init_distributed_mode(args):
    ...
    # launched naively with `python main_lava.py`
    # we manually add MASTER_ADDR and MASTER_PORT to env variables
    elif torch.cuda.is_available():
        print('Will run the code on one GPU.')
        args.rank, args.gpu, args.world_size = 0, 0, 1
        os.environ['MASTER_ADDR'] = '127.0.0.1'
        os.environ['MASTER_PORT'] = args.dist_master_port if hasattr(args, 'dist_master_port') else '29500'
    else:
...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        for end_idx in idx_crops:
            _out1, _out2, _out3 = self.head(self.backbone(torch.cat(x[start_idx: end_idx])))
            if start_idx == 0:
                output1, output2, output3 = _out1, _out2, _out3
            else:
                output1, output2, output3 = torch.cat((output1, _out1)), torch.cat((output2, _out2)), torch.cat((output3, _out3))
            start_idx = end_idx
...
def get_params_groups(model):
    ...
    print(f'No Grad params: {len(no_grad)}, Regularized params: {len(regularized)}, '
          f'Non-regularized params: {len(not_regularized)}')
...
def get_labels2wv_dict(original_labels, wv_dict, return_mapping=False, check_edit_dist=False):
    """
            ...
            # to handle regular plural form, convert to singular form by removing s and check
                ...
                if 'of' in words:
                    ...
                    try:
                        ...
                        nltk.download('averaged_perceptron_tagger', download_dir=Path.home())
                        pos_tag = nltk.pos_tag(words)
...
def fuse_predictions(preds_1, preds_2, preds_3, tie_breaker=2):
    assert len(preds_1) == len(preds_2) == len(preds_3), 'fused predictions must be of the same size'
    fused = []
    for first, second, third in zip(preds_1, preds_2, preds_3):
        preds = [first, second, third]
        if len(preds) == len(set(preds)):
            fused.append(first if tie_breaker == 1 else second if tie_breaker == 2 else third)
        else:
            counter = {k: preds.count(k) for k in set(preds)}
            fused.append(list(counter.keys())[np.argmax(list(counter.values()))])
    return fused
